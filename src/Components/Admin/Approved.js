...
const Approved = ({ onStart }) => {
  ...
  const approveProduct = (productId) => {
    fetch(`http://localhost:8000/product/${productId}/approve`, {
      method: "PUT",
      headers: {
  ...
  const rejectProduct = (productId) => {
    fetch(`http://localhost:8000/product/${productId}/reject`, {
      method: "PUT",
    })
      .then((res) => res.json())
      .then((result) => {
  ...
  const approveRole = (roleId, newRole) => {
    fetch(`http://localhost:8000/user/approve/${roleId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((result) => {
        ...
        setRoles(
          roles.map((role) => {
            if (role._id === roleId) {
              return { ...role, status: "approved", role: newRole };
            }
            return role;
          })
        );
  ...
  const rejectRole = (roleId) => {
    fetch(`http://localhost:8000/user/${roleId}/reject`, {
      method: "PUT",
    })
      .then((res) => res.json())
      .then((result) => {
            ...
            <div class="w-80 bg-white shadow-lg rounded-lg overflow-hidden my-4">
              <div className="flex justify-between mx-auto gap-5">
                <img
                  ...
                  src={`http://localhost:8000/uploads/${role.image1}`}
                  alt="image1"
                />
                <img
                  className="w-32 h-40 object-cover object-center"
                  src={`http://localhost:8000/uploads/${role.image2}`}
                  alt="image2"
                />
              </div>
              <div class="py-4 px-6">
